...
bool ShouldPredict()
{
	...
	if (server_time >= (G::PredictedTime) && !G::LocalPlayer->IsMoving() && *G::LocalPlayer->GetFlags() & FL_ONGROUND)
	{
...
void BreakLowerbodyFreestand()
{
	...
	if (G::SendPacket) {
		G::UserCmd->viewangles.y = BestHeadPosition + LowerbodyDelta + Math::RandomFloat2(-6.f, 6.f);
	}
	else {
		if (ShouldPredict())
...
void CAntiaim::Fakewalk(CUserCmd* userCMD)
{
	...
	else
	{
		...
		if (iChoked < 1)
		{
			...
			userCMD->tick_count += 10.95; // 10.95
			userCMD->command_number += 5.07 + userCMD->tick_count % 2 ? 0 : 1; // 5

			userCMD->buttons |= G::LocalPlayer->GetMoveType() == IN_BACK;
			userCMD->forwardmove = userCMD->sidemove = 0.f;
		}
		else
		{
			G::SendPacket = true;
			...
			g_pGlobals->frametime *= (G::LocalPlayer->GetVelocity().Length2D()) / 10; // 10
			userCMD->buttons |= G::LocalPlayer->GetMoveType() == IN_FORWARD;
		}
	}
}
