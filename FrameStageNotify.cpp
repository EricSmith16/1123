					...
					if (Clientvariables->Ragebot.AutomaticResolver) {
						{
							...
							Yaw[idx] = pEntity->GetEyeAnglesPtr()->y;
							{
								...
								else if (velocity > 20 && velocity < 50 && !ACT_STAND)(pEntity);
								{
									if (missedshots[target] > 2 || pEntity)
									{
										lastLby[idx] = Yaw[idx] - 15 * missedshots[target];
										lastLby[idx] = Yaw[idx] + 40 * missedshots[target];
										lastLby[idx] = Yaw[idx] - 40 * missedshots[target];
										lastLby[idx]  = Yaw[idx] + 15 * missedshots[target];
									}
								}
